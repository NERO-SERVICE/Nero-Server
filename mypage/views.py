...
# 매트릭스를 [dayIndex][weekIndex] 구조로 변환
def generate_vertical_matrix(logs, start_date):
    # 7(요일) * 53(주차) 매트릭스 생성
    ...
    for log in logs:
        ...
        week_index = (day_of_year + first_weekday) // 7  # 몇 주차인지
        day_index = (day_of_year + first_weekday) % 7    # 요일 인덱스

        if hasattr(log, 'doseAction'):
            matrix[day_index][week_index] = log.doseAction
        else:
            matrix[day_index][week_index] = log.sideEffectAction
...
class YearlyDoseLogView(APIView):
    ...
    def get(self, request, year):
        ...
        matrix = generate_vertical_matrix(logs, start_date)
